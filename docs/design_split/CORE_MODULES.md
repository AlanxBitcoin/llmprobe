...
**新增：Single Word Hidden-State Heatmap 视图规范**：
- 左侧新增按钮：`Single Word Hidden State`
- 中部参数表单新增字段：`word`（单词输入）
- 点击执行后：对该词运行一次模型前向，提取 `embedding + 36 layers`，共 `37 x 4096` 矩阵
- UI 渲染为热力图：
  - 行数：`37`（建议第 0 行标记为 embedding，1-36 行为层）
  - 列数：`4096`（神经元维度）
  - 单元格尺寸：`10 x 10 px`
  - 颜色映射：`0 = 黑色`，`正值 = 红色`，`负值 = 蓝色`，绝对值越大颜色越深
- 画布必须支持横向滚动，从第 `0` 维浏览到第 `4095` 维；不做压缩聚合，不丢维度
